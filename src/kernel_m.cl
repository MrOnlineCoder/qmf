...
__kernel void compute(__global int *N, __global int *result,
                      __global ulong *offset, __global int *dualCount,
                      __global ulong *hist) {
  ...
  for (ulong i = 0; i < vc; i++) {
    ...
    f[fidx] = value;
    // copy the values right away to direct/inverse function buffers
    fd[fidx] = value;
    fi[fidx] = value;
  ...
  if (isDual) {
    hist[*dualCount] = nf;
    atomic_add(dualCount, 1);
  }

  // for (int i = 0; i < vc; i++) {
  //   atomic_add(hist + i, f[i]);
  // }
}
